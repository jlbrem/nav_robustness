...
fairseq=~/fairseq
m2m=~/models/m2m
data=~/projects/duo21/seeds/prepped
proj=~/projects/duo21/seeds
save=$proj/finetune

lang=$1
seed=$2

mkdir -p $save/$lang-en
mkdir -p $save/$lang-en/$seed

conda activate duo21

lang_pairs=$lang-en
path_2_data=$data/$lang-en/$seed
lang_list=$m2m/language_pairs_small_models.txt
pretrained_model=$m2m/1.2B_last_checkpoint.pt

fairseq-train $path_2_data \
  --finetune-from-model $pretrained_model --save-dir $save/$lang-en/$seed \
  ...
  --lang-pairs $lang-en --max-tokens 1200 --decoder-normalize-before --sampling-method temperature \
  ...
  --keep-interval-updates 10 --no-epoch-checkpoints --seed 222 --log-format simple --log-interval 2 --patience 10 \
